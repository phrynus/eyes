...
module.exports = async (obj) => {
  try {
    if (!obj.params.symbol || !obj.params.position || !obj.params.side || !obj.params.quantity || obj.bin.safe_tradeList[coin].type === 'LIMIT' ? !obj.params.price : false) {
    ...
    const bin_params = {
      ...
      quantity: Number(obj.params.quantity).toFixed(exchangeInfoSymbol.quantityPrecision),
      timestamp: null,
    };
    // 如果是限价单，设置价格和有效期
    ...
    obj.newKeyLog.save();
    logger.info(`[${obj.newKeyLog._id}][ANALYSIS][ORDER][交易完成] ${JSON.stringify(obj.params)} > ${JSON.stringify(bin_result)}`);

    //
  } catch (err) {
    logger.error(`[错误][${obj.newKeyLog._id}][ANALYSIS][ORDER][${err.message}] > ${JSON.stringify(obj.params)}`);
    logger.error(err);
