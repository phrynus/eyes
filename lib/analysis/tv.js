...
module.exports = async (obj) => {
  try {
    ...
    // 获取交易对的交易所信息
    const exchangeInfoSymbol = config.bin.binance.exchangeInfo.symbols.find((item) => item.symbol === coin);
    ...
    if (obj.bin.safe_tradeList[coin].type === 'MARKET') {
      // 市价单
      if (MARKET_maxQty < Number(obj.params.quantity) || MARKET_minQty > Number(obj.params.quantity)) {
        throw new Error('数量限制');
      }
    } else {
      // 检查数量和价格是否在有效范围内
      if (minQty > Number(obj.params.quantity) || maxQty < Number(obj.params.quantity)) {
        throw new Error('数量限制');
      }
      // 检查数量和价格是否在有效范围内
      if (minPrice > Number(obj.params.price) || maxPrice < Number(obj.params.price)) {
    ...
    const bin_params = {
      ...
      positionSide: obj.params.market_position === 'flat' ? obj.params.prev_market_position.toUpperCase() : obj.params.market_position.toUpperCase(),
      side: obj.params.action.toUpperCase(),
      quantity: Number(obj.params.contracts).toFixed(exchangeInfoSymbol.quantityPrecision),
      timestamp: null,
    };
    // 如果是限价单，设置价格和有效期
    if (obj.bin.safe_tradeList[coin].type === 'LIMIT') {
      bin_params.price = Number(obj.params.price).toFixed(exchangeInfoSymbol.pricePrecision);
    ...
    const funActions = {
      开单: async () => {
        ...
        config.coins[obj.bin.markId].includes(coin) ? null : config.coins[obj.bin.markId].push(coin);
      },
      平单: async () => {
        /* 平单操作 */
    ...
    logger.info(`[${obj.newKeyLog._id}][ANALYSIS][TV][${PositionRes.actionsType}][交易完成] ${JSON.stringify(obj.params)} > ${JSON.stringify(bin_result)}`);
  } catch (err) {
    logger.error(`[错误][${obj.newKeyLog._id}][ANALYSIS][TV][${err.message}] > ${JSON.stringify(obj.params)}`);
    logger.error(err);
